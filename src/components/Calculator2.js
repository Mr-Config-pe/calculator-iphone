...
function Calculator() {
  ...
  const handleClick = (value) => {
    if (display.length < 15) {
      let newDisplay;
      if (display === "0" && (value === "." || !operators.includes(value))) {
        newDisplay = value;
      } else if (["+", "-", "*", "/"].includes(value) && operator === null) {
        setOperator(value);
        setFirstNum(firstNum === null ? display : firstNum);
        newDisplay = firstNum === null ? value : firstNum + value;
      } else if (
      ...
      } else if (["+", "-", "*", "/"].includes(display[display.length - 1])) {
        newDisplay = display.slice(0, display.length - 1) + value;
      } else {
  ...
  const performCalculation = () => {
    ...
    if (operator === "+") {
      setDisplay((Number(firstNum) + Number(secondNum)).toString());
    } else if (operator === "-") {
      setDisplay((Number(firstNum) - Number(secondNum)).toString());
    } else if (operator === "*") {
      setDisplay((Number(firstNum) * Number(secondNum)).toString());
      ...
      if (secondNum === 0) {
        setDisplay("Error: División por cero");
      } else {
        setDisplay((Number(firstNum) / Number(secondNum)).toString());
      }
    }
